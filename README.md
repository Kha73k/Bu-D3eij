...
- Logo-derived red theme (Dark by default; switch Light/Dark/System in the sidebar)
...
> `.pptx` files (legacy `.ppt` is not supported).
...
- **ffmpeg** — required for the audio/video conversions.
...
Convert a single file without opening the window:
...
ffmpeg still needs to be installed for audio/video conversions.
...
pymupdf4llm · mammoth · markdownify · ffmpeg-python
